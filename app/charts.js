...
function Charts({weatherData, rendered}) {
  ...
  const getHourFromDates = (dates) => {
    const hours = [];
    for (const date of dates) {
      const datetimeObject = new Date(date);
      hours.push(datetimeObject.getHours());
    }
    return hours;
  };
  ...
  <div className="container px-5 py-4 mx-auto">
    <div className="flex flex-wrap -m-4 text-center">
      <div className="p-4 sm:w-1/4 w-1/2">
        <h2 className="title-font font-medium sm:text-4xl text-3xl text-white">{sunrise ? Math.round(average(getHourFromDates(sunrise))): ''}</h2>
        <p className="leading-relaxed">Sunrise (Avg Hour)</p>
      </div>
      <div className="p-4 sm:w-1/4 w-1/2">
        <h2 className="title-font font-medium sm:text-4xl text-3xl text-white">{sunset ? Math.round(average(getHourFromDates(sunset))): ''}</h2>
        <p className="leading-relaxed">Sunset (Avg Hour)</p>
      </div>
      <div className="p-4 sm:w-1/4 w-1/2">
        <h2 className="title-font font-medium sm:text-4xl text-3xl text-white">{temperatureApparentAvg ? Math.round(average(temperatureApparentAvg)) : ''}</h2>
        <p className="leading-relaxed">Avg Temp (C)</p>
      </div>
      <div className="p-4 sm:w-1/4 w-1/2">
        <h2 className="title-font font-medium sm:text-4xl text-3xl text-white">{windDirectionAvg ? Math.round(average(windDirectionAvg)): ''}</h2>
